...
@app.post("/crawled_data/", response_model=schemas.CrawledData, status_code=201, tags = ['Crawled Data'])
...
@app.get("/crawled_data/", response_model=list[schemas.CrawledData],tags = ['Crawled Data'])
...
@app.get("/crawled_data/{crawled_data_id}/", response_model=schemas.CrawledData ,tags = ['Crawled Data'])
...
def read_jobs_by_user(user_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
